...
pcb_t* get_highest_priority_process(){
...
return &pcb[highest_priority_index];
}
...
void priority_queue_schedule( ctx_t* ctx ) {
    ...
    pcb_t* highest_priority_process = get_highest_priority_process();
    if ( (highest_priority_process->priority > current->priority) && (highest_priority_process->status != STATUS_TERMINATED) && (highest_priority_process != current)){
        //Reset the age
        highest_priority_process->priority = highest_priority_process->base_priority;
        //Swap to next process
        dispatch(ctx, current, highest_priority_process);
        current->status = STATUS_READY;
        highest_priority_process->status = STATUS_EXECUTING;
    }
    return;
}
...
//Needed in case the index =/= the pid
...
void sys_kill(ctx_t* ctx){
    pid_t target_pid = ctx->gpr[0];
    int sig_term = ctx->gpr[1];
    pcb_t* target_process = get_process_from_pid(target_pid);
    if (target_process == NULL) {
        //Process not found
    } else if (target_process->status == STATUS_TERMINATED){
        //Process already terminated
    } else {
        terminate_process(target_process, ctx);
    }
}
